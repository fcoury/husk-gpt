...
use crate::ast::{Export, ExportItem, ImportClause, ImportItem};
use crate::{Spanned, Token, ast::*};
...
impl Parser {
    ...
    fn parse_primary_type(&mut self) -> Option<Type> {
        match &self.peek()?.value {
            ...
            Token::LeftParen => {
                ...
                // Single type in parentheses is just grouped, not a tuple
                if types.len() == 1 {
    ...
    fn parse_primary_expression(&mut self) -> Option<Expression> {
        match &self.peek()?.value {
            ...
            Token::Match => {
                ...
                Some(Expression::Match(MatchExpression { scrutinee, arms }))
            }
            _ => {
    ...
    fn parse_struct(&mut self, vis: Visibility, start_span: Span) -> Option<Spanned<Item>> {
        ...
        Some(Spanned::new(
            Item::Struct(Struct { vis, name, fields }),
            span,
        ))
    }
    ...
    fn parse_type_alias(&mut self, vis: Visibility, start_span: Span) -> Option<Spanned<Item>> {
        ...
        Some(Spanned::new(
            Item::TypeAlias(TypeAlias { vis, name, ty }),
            span,
        ))
    }
    ...
    fn parse_interface(&mut self, vis: Visibility, start_span: Span) -> Option<Spanned<Item>> {
        ...
        while !self.check(&Token::RightBrace) && !self.is_at_end() {
            ...
            let _ = self.match_token(&Token::Comma) || self.match_token(&Token::Semicolon);
        }
        ...
        Some(Spanned::new(
            Item::Interface(Interface { vis, name, methods }),
            span,
        ))
    }
    ...
    fn parse_import(&mut self, start_span: Span) -> Option<Spanned<Item>> {
        ...
        Some(Spanned::new(Item::Import(Import { path, items }), span))
    }
    ...
    fn parse_export(&mut self, start_span: Span) -> Option<Spanned<Item>> {
        ...
        Some(Spanned::new(Item::Export(export), span))
    }
